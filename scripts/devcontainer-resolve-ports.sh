...
ports_unique() {
  local ports=("$FRONTEND_PORT" "$BACKEND_PORT" "$BACKEND_OSS_GATEWAY_PORT" "$POSTGRES_PORT" "$POSTGRES_TEST_PORT" "$MAILHOG_SMTP_PORT" "$MAILHOG_UI_PORT")
  local i j
...
ports_ok() {
  ports_unique \
    ...
    && port_available "$MAILHOG_SMTP_PORT" \
    && port_available "$MAILHOG_UI_PORT"
}
...
if [[ -f "$ENV_FILE" ]] && ports_ok; then
  echo "✅ 使用已有端口: frontend=${FRONTEND_PORT} backend=${BACKEND_PORT} oss_gateway=${BACKEND_OSS_GATEWAY_PORT} postgres=${POSTGRES_PORT} test=${POSTGRES_TEST_PORT} mailhog_smtp=${MAILHOG_SMTP_PORT} mailhog_ui=${MAILHOG_UI_PORT}"
else
  FRONTEND_PORT="$(find_free_port "${FRONTEND_PORT:-3600}")"
  BACKEND_PORT="$(find_free_port "${BACKEND_PORT:-8600}" "$FRONTEND_PORT")"
  BACKEND_OSS_GATEWAY_PORT="$(find_free_port "${BACKEND_OSS_GATEWAY_PORT:-8610}" "$FRONTEND_PORT" "$BACKEND_PORT")"
  POSTGRES_PORT="$(find_free_port "${POSTGRES_PORT:-5600}" "$FRONTEND_PORT" "$BACKEND_PORT" "$BACKEND_OSS_GATEWAY_PORT")"
  POSTGRES_TEST_PORT="$(find_free_port "${POSTGRES_TEST_PORT:-5610}" "$FRONTEND_PORT" "$BACKEND_PORT" "$BACKEND_OSS_GATEWAY_PORT" "$POSTGRES_PORT")"
  MAILHOG_SMTP_PORT="$(find_free_port "${MAILHOG_SMTP_PORT:-8630}" "$FRONTEND_PORT" "$BACKEND_PORT" "$BACKEND_OSS_GATEWAY_PORT" "$POSTGRES_PORT" "$POSTGRES_TEST_PORT")"
  MAILHOG_UI_PORT="$(find_free_port "${MAILHOG_UI_PORT:-8650}" "$FRONTEND_PORT" "$BACKEND_PORT" "$BACKEND_OSS_GATEWAY_PORT" "$POSTGRES_PORT" "$POSTGRES_TEST_PORT" "$MAILHOG_SMTP_PORT")"
  write_env
  echo "✅ 已分配端口: frontend=${FRONTEND_PORT} backend=${BACKEND_PORT} oss_gateway=${BACKEND_OSS_GATEWAY_PORT} postgres=${POSTGRES_PORT} test=${POSTGRES_TEST_PORT} mailhog_smtp=${MAILHOG_SMTP_PORT} mailhog_ui=${MAILHOG_UI_PORT}"
fi
